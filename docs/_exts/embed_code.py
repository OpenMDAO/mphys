...
class EmbedCodeDirective(Directive):
    ...
    def run(self):
        ...
        is_test = (
            class_ is not None
            and inspect.isclass(class_)
            and issubclass(class_, unittest.TestCase)
        )
        ...
        if is_test:
            try:
                ...
                if "interleave" in layout:
                    ...
                    code_to_run = "\n".join(
                        [self_code, setup_code, interleaved, teardown_code]
                    ).strip()
                else:
                    code_to_run = "\n".join(
        ...
        if failed:
            ...
            if "output" in layout:
                output_blocks = (
                    run_outputs if isinstance(run_outputs, list) else [run_outputs]
                )
