...
class DistributedConverter(om.ExplicitComponent):
    ...
    def setup(self):
        for input in self.options["distributed_inputs"]:
            ...
            self.add_input(
                input.name, shape=in_shape, tags=input.tags, distributed=True
            )
        ...
        for output in self.options["distributed_outputs"]:
            ...
            self.add_output(
                output.name, shape=out_shape, tags=output.tags, distributed=True
            )
    ...
    def compute(self, inputs, outputs):
        for input in self.options["distributed_inputs"]:
            ...
            outputs[f"{input.name}_serial"] = self.comm.bcast(
                outputs[f"{input.name}_serial"]
            )
