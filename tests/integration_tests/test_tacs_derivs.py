...
class Top(Multipoint):
    def setup(self):
        ...
        self.connect(
            "f_struct", f"analysis.{MPhysVariables.Structures.Loads.AERODYNAMIC}"
        )
...
class TestTACS(unittest.TestCase):
    ...
    @unittest.skipUnless(
        TACS.dtype == complex, "TACS must be compiled in complex mode."
    )
    def test_derivatives(self):
        self.prob.run_model()
        print("----------------starting check totals--------------")
        data = self.prob.check_totals(
            of=["analysis.ks_vmfailure", "analysis.mass"],
            wrt=f"mesh.fea_mesh.{MPhysVariables.Structures.Mesh.COORDINATES}",
            method="cs",
            step=1e-50,
            step_calc="rel",
        )
