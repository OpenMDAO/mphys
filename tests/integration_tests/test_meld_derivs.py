...
class TestXferClasses(unittest.TestCase):
    def setUp(self):
        ...
        prob.model.connect(
            "aero_mesh.x_aero", ["disp_xfer.x_aero0", "load_xfer.x_aero0"]
        )
        prob.model.connect(
            "struct_mesh.x_struct0", ["disp_xfer.x_struct0", "load_xfer.x_struct0"]
        )
        prob.model.connect(
            "struct_disps.u_struct", ["disp_xfer.u_struct", "load_xfer.u_struct"]
        )
        ...
        prob.model.connect(
            "aero_mesh2.x_aero", ["disp_xfer2.x_aero0", "load_xfer2.x_aero0"]
        )
        prob.model.connect(
            "struct_mesh2.x_struct0", ["disp_xfer2.x_struct0", "load_xfer2.x_struct0"]
        )
        prob.model.connect(
            "struct_disps2.u_struct", ["disp_xfer2.u_struct", "load_xfer2.u_struct"]
        )
    ...
    @unittest.skipUnless(
        TransferScheme.dtype == complex, "FunToFem must be compiled in complex mode."
    )
        ...
        for key, comp in data.items():
            for var, err in comp.items():
                ...
                if var[1] == "f_aero" or var[1] == "u_struct":
                    assert_near_equal(rel_err.forward, 0.0, 1e-11)
