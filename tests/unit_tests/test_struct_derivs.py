...
class Top(om.Group):
    ...
    def configure(self):
        ...
        self.dvs.add_output("f_s", np.ones(f_size))

        self.connect("f_s", ["mp.s0.solver_group.struct.f_s"])

        self.mp.struct_mesh.mphys_add_coordinate_input()
        self.dvs.add_output("xpts", self.mp.struct_mesh.xpts.getArray())

        self.connect("xpts", ["mp.struct_mesh.x_s0_points"])
...
class TestTACs(unittest.TestCase):
    def setUp(self):
        ...
        # prob.model.add_objective("mp.s0.struct_funcs.mass.mass", ref=100.0)
    ...
    def test_derivatives(self):
        ...
        data = self.prob.check_totals(wrt="f_s", step=3e-5, step_calc="rel")  # out_stream=None
        for var, err in data.items():
