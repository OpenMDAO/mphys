...
class Geometry(om.ExplicitComponent):
    ...
    def setup(self):
        for disp, size in zip(
            self.options["disciplines"], self.options["discipline_num_nodes"]
        ):
            self.add_input(
                f"x_{disp}0_geometry_input",
                shape_by_conn=True,
                tags=["mphys_coordinates"],
            )
            self.add_output(
                f"x_{disp}0_geometry_output", shape=3 * size, tags=["mphys_coordinates"]
            )
...
class GeometryBuilder(Builder):
    ...
    def get_mesh_coordinate_subsystem(self, scenario_name=None):
        return Geometry(
            disciplines=self.disciplines, discipline_num_nodes=self.num_nodes
        )
