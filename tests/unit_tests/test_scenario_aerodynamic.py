...
class TestScenarioAerodynamic(unittest.TestCase):
    def setUp(self):
        ...
        self.prob.model.add_subsystem(
            "scenario", ScenarioAerodynamic(aero_builder=builder)
        )
...
class TestScenarioAerodynamicParallel(unittest.TestCase):
    def setUp(self):
        ...
        self.prob.model.add_subsystem(
            "scenario",
            ScenarioAerodynamic(aero_builder=builder, in_MultipointParallel=True),
        )
        self.prob.setup()
...
class TestScenarioAerodynamicParallelWithGeometry(unittest.TestCase):
    ...
    def test_coordinates_connected_from_geometry(self):
        ...
        for sys in systems:
            self.assertEqual(
                scenario._conn_global_abs_in2out[
                    f"scenario.{sys}.{MPhysVariables.Aerodynamics.Surface.COORDINATES_INITIAL}"
                ],
        ...
        for sys in systems:
            self.assertEqual(
                scenario._conn_global_abs_in2out[
                    f"scenario.{sys}.{MPhysVariables.Aerodynamics.Surface.COORDINATES}"
                ],
                f"scenario.geometry.{MPhysVariables.Aerodynamics.Surface.Geometry.COORDINATES_OUTPUT}",
            )
