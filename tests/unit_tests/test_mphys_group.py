...
class TestMPhysGroupAddingMphysSubsystem(unittest.TestCase):
    ...
    def test_tagged_input_promotion(self):
        inputs = self.prob.model.mphys_group.get_io_metadata('input',
                                                             metadata_keys=['tags'])
        for key, val in inputs.items():
            if ('mphys_input' in val['tags'] or 'mphys_coupling' in val['tags'] or
                    'mphys_coordinates' in val['tags']):
                self.assertEqual(val['prom_name'], key.split('.')[-1])
            else:
                self.assertEqual(val['prom_name'], key)

    def test_tagged_output_promotion(self):
        inputs = self.prob.model.mphys_group.get_io_metadata('output',
                                                             metadata_keys=['tags'])
        for key, val in inputs.items():
            if ('mphys_result' in val['tags'] or 'mphys_coupling' in val['tags'] or
                    'mphys_coordinates' in val['tags']):
                self.assertEqual(val['prom_name'], key.split('.')[-1])
            else:
                self.assertEqual(val['prom_name'], key)
