    ...
    def _mphys_scenario_setup(self):
        for builder in self.options["builders"]:
            self._mphys_add_pre_coupling_subsystem_from_builder(
                builder.name, builder, self.name
            )
...
class TestScenario(unittest.TestCase):
    def setUp(self) -> None:
        ...
        scenario.mphys_add_post_subsystem(
            "user_post",
            UserPostCompPromoteInputsAndOutputs(),
            promotes_inputs=["out_post"],
            promotes_outputs=["out_user"],
        )
        scenario.mphys_add_post_subsystem("user_post2", UserPostCompMphysPromote())
        scenario.mphys_add_post_subsystem(
            "user_post3", UserPostCompUseGeneralPromotes(), promotes=["*"]
        )
    ...
    def test_components_were_added(self):
        ...
        self.assertIsInstance(
            self.prob.model.scenario.user_post, UserPostCompPromoteInputsAndOutputs
        )
        self.assertIsInstance(
            self.prob.model.scenario.user_post2, UserPostCompMphysPromote
        )
        self.assertIsInstance(
            self.prob.model.scenario.user_post3, UserPostCompUseGeneralPromotes
        )
