...
class MeshComp(om.IndepVarComp):
    def setup(self):
        self.add_output(
            "x_aero0", val=np.ones(num_nodes * 3), tags=["mphys_coordinates"]
        )
...
class PostCouplingComp(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["func_aero"] = np.sum(
            inputs["f_aero"] + inputs["prestate_aero"] + inputs["x_aero"]
        )
    ...
    def setUp(self):
        ...
        for scenario in self.scenarios:
            self.prob.model.add_subsystem(
                scenario,
                ScenarioAerodynamic(aero_builder=builder, run_directory=scenario),
            )
