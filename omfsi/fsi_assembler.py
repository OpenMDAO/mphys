...
class FsiAssembler(object):
    ...
    def add_fsi_subsystems(self,model,scenario):
        fsi_group = scenario.add_subsystem('fsi_group',Group())

        self.connection_srcs = {}

        self.xfer_assembler.add_components(model,scenario,fsi_group,self.connection_srcs)
        self.geodisp_assembler.add_components(model,scenario,fsi_group,self.connection_srcs)
        self.struct_assembler.add_components(model,scenario,fsi_group,self.connection_srcs)
        self.aero_assembler.add_components(model,scenario,fsi_group,self.connection_srcs)
...
class GeoDispAssembler(object):
    def add_components(self,model,scenario,fsi_group,connection_srcs):
        fsi_group.add_subsystem('geo_disp',GeoDip())
