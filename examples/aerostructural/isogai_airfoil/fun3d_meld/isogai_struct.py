...
class IsogaiEigenComp(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('alpha_m', tags=['mphys_input'])
        self.add_input('alpha_k', tags=['mphys_input'])
        ...
        self.add_input('x_struct0', shape_by_conn=True,
                       distributed=True, tags=['mphys_coordinates'])

        self.add_output(
            'mode_shapes_struct', shape=[self.nnodes * 3, self.nmodes],
            distributed=True, tags=['mphys_coupling'])
    ...
    def compute(self, inputs, outputs):
        mass, stiff, vec0, vec1 = self._structural_eigen_decomposition(inputs['plunge_frequency'],
                                                                       inputs['pitch_frequency'])
        outputs['modal_mass'] = mass
        outputs['modal_stiffness'] = stiff
        outputs['modal_damping'] = (inputs['alpha_m']*outputs['modal_mass']
                                  + inputs['alpha_k']*outputs['modal_stiffness'])
    ...
    def compute_jacvec_product(self, inputs, d_inputs, d_outputs, mode):
        if 'alpha_m' in d_inputs:
            mass, stiff, vec0, vec1 = self._structural_eigen_decomposition(inputs['plunge_frequency'],
                                                                           inputs['pitch_frequency'])
            if 'modal_damping' in d_outputs:
                if mode == 'fwd':
                    d_outputs['modal_damping'] += mass * d_inputs['alpha_m']
                if mode == 'rev':
                    d_inputs['alpha_m'] += np.sum(mass * d_outputs['modal_damping'])

        if 'alpha_k' in d_inputs:
            mass, stiff, vec0, vec1 = self._structural_eigen_decomposition(inputs['plunge_frequency'],
                                                                           inputs['pitch_frequency'])
            if 'modal_damping' in d_outputs:
                if mode == 'fwd':
                    d_outputs['modal_damping'] += stiff * d_inputs['alpha_k']
                if mode == 'rev':
                    d_inputs['alpha_k'] += np.sum(stiff * d_outputs['modal_damping'])

        if 'plunge_frequency' in d_inputs:
            perturb = 1e-6
            mass, stiff, vec0, vec1 = self._structural_eigen_decomposition(inputs['plunge_frequency'],
                                                                           inputs['pitch_frequency'])
            pmass, pstiff, pvec0, pvec1 = self._structural_eigen_decomposition(inputs['plunge_frequency'] + perturb,
                                                                               inputs['pitch_frequency'])
            ...
            if 'modal_damping' in d_outputs:
                if mode == 'fwd':
                    d_outputs['modal_damping'] += dmass * inputs['alpha_m'] * d_inputs['plunge_frequency']
                    d_outputs['modal_damping'] += dstiff * inputs['alpha_k'] * d_inputs['plunge_frequency']
                if mode == 'rev':
                    d_inputs['plunge_frequency'] += np.sum(dmass * inputs['alpha_m'] * d_outputs['modal_damping'])
                    d_inputs['plunge_frequency'] += np.sum(dstiff * inputs['alpha_k'] * d_outputs['modal_damping'])
        ...
        if 'pitch_frequency' in d_inputs:
            ...
            mass, stiff, vec0, vec1 = self._structural_eigen_decomposition(inputs['plunge_frequency'],
                                                                           inputs['pitch_frequency'])
            pmass, pstiff, pvec0, pvec1 = self._structural_eigen_decomposition(inputs['plunge_frequency'],
                                                                               inputs['pitch_frequency']+perturb)
            dmass = (pmass - mass) / perturb
            dstiff = (pstiff - stiff) / perturb

            if 'modal_damping' in d_outputs:
                if mode == 'fwd':
                    d_outputs['modal_damping'] += dmass * inputs['alpha_m'] * d_inputs['pitch_frequency']
                    d_outputs['modal_damping'] += dstiff * inputs['alpha_k'] * d_inputs['pitch_frequency']
                if mode == 'rev':
                    d_inputs['pitch_frequency'] += np.sum(dmass * inputs['alpha_m'] * d_outputs['modal_damping'])
                    d_inputs['pitch_frequency'] += np.sum(dstiff * inputs['alpha_k'] * d_outputs['modal_damping'])
            if 'modal_mass' in d_outputs:
                if mode == 'fwd':
...
class ModalSolver(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('modal_stiffness', shape_by_conn=True,
                       desc='modal stiffness', tags=['mphys_coupling'])
...
class ModalForces(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('mode_shapes_struct', shape_by_conn=True,
                       distributed=True, desc='structural mode shapes')
        self.add_input('f_struct', shape_by_conn=True, distributed=True,
                       tags=['mphys_coupling'], desc='nodal force')
        ...
        if self.load_size > 0:
            for imode in range(self.nmodes):
                outputs['mf'][imode] = np.sum(
                    inputs['mode_shapes_struct'][:, imode] * inputs['f_struct'][:])
                ...
                if self.load_size > 0:
                    ...
                    if 'f_struct' in d_inputs:
                        for imode in range(self.options['nmodes']):
                            d_outputs['mf'][imode] += np.sum(inputs['mode_shapes_struct']
                                                             [:, imode] * d_inputs['f_struct'][:])

                    if 'mode_shapes_struct' in d_inputs:
                        for imode in range(self.options['nmodes']):
                            d_outputs['mf'][imode] += np.sum(d_inputs['mode_shapes_struct']
                                                             [:, imode] * inputs['f_struct'][:])
        ...
        if mode == 'rev':
            if 'mf' in d_outputs:
                if self.load_size > 0:
                    ...
                    if 'f_struct' in d_inputs:
                        for imode in range(self.options['nmodes']):
                            d_inputs['f_struct'][:] += inputs['mode_shapes_struct'][:,
                                                                                    imode] * d_outputs['mf'][imode]

                    if 'mode_shapes_struct' in d_inputs:
                        for imode in range(self.options['nmodes']):
                            d_inputs['mode_shapes_struct'][
                                :, imode] += inputs['f_struct'][:] * d_outputs['mf'][imode]
...
class ModalDisplacements(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('mode_shapes_struct', shape_by_conn=True, distributed=True,
                       desc='structural mode shapes', tags=['mphys_coupling'])
        self.add_input('z', shape_by_conn=True, desc='modal displacement')

        self.add_output('u_struct', np.zeros(self.mode_size), distributed=True,
                        desc='nodal displacement', tags=['mphys_coupling'])
        ...
        if mode == 'fwd':
            if 'u_struct' in d_outputs:
                if self.mode_size > 0:
                    ...
                    if 'z' in d_inputs:
                        for imode in range(self.options['nmodes']):
                            d_outputs['u_struct'][:] += inputs['mode_shapes_struct'][:,
                                                                                     imode] * d_inputs['z'][imode]

                    if 'mode_shapes_struct' in d_inputs:
                        for imode in range(self.options['nmodes']):
                            d_outputs['u_struct'][:] += d_inputs['mode_shapes_struct'][:,
                                                                                       imode] * inputs['z'][imode]
        ...
        if mode == 'rev':
            if 'u_struct' in d_outputs:
                ...
                if 'z' in d_inputs:
                    if self.mode_size > 0:
                        for imode in range(self.options['nmodes']):
                            d_inputs['z'][imode] += np.sum(inputs['mode_shapes_struct']
                                                           [:, imode] * d_outputs['u_struct'][:])
                ...
                if 'mode_shapes_struct' in d_inputs:
                    if self.mode_size > 0:
                        for imode in range(self.options['nmodes']):
                            d_inputs['mode_shapes_struct'][
                                :, imode] += inputs['z'][imode] * d_outputs['u_struct'][:]
...
if __name__ == '__main__':
    ...
    ivc.add_output('alpha_k', 0.005)
    ivc.add_output('alpha_m', 1.0)
