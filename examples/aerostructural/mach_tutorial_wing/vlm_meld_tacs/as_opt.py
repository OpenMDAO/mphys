...
from struct_dv_components import SmoothnessEvaluatorGrid, StructDvMapper, struct_comps
...
class Top(Multipoint):
    def setup(self):
        ...
        dvs.add_output(
            "up_stringer", val=initial_thickness, shape=struct_comps["up_stringer"]
        )
        dvs.add_output(
            "lo_stringer", val=initial_thickness, shape=struct_comps["lo_stringer"]
        )
        ...
        self.add_subsystem(
            "le_spar_smoothness",
            SmoothnessEvaluatorGrid(columns=struct_comps["le_spar"], rows=1),
        )
        self.add_subsystem(
            "te_spar_smoothness",
            SmoothnessEvaluatorGrid(columns=struct_comps["te_spar"], rows=1),
        )
        self.add_subsystem(
            "up_skin_smoothness",
            SmoothnessEvaluatorGrid(columns=9, rows=struct_comps["up_skin"] // 9),
        )
        self.add_subsystem(
            "lo_skin_smoothness",
            SmoothnessEvaluatorGrid(columns=9, rows=int(struct_comps["lo_skin"] / 9)),
        )
...
prob.model.add_design_var(
    "aoa", lower=-5 * np.pi / 180, upper=10 * np.pi / 180.0, ref=1.0, units="rad"
)
...
if check_derivs:
    prob.run_model()
    prob.check_totals(
        of=["cruise.mass", "cruise.C_L", "maneuver.ks_vmfailure"], wrt=["aoa", "ribs"]
    )
else:
    prob.run_driver()
