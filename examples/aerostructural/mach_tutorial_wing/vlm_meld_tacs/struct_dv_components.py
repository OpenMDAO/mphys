...
class SmoothnessEvaluatorGrid(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.ndiffs = 2 * (self.columns - 1) * self.rows + 2 * self.columns * (
            self.rows - 1
        )
    ...
    def compute(self, inputs, outputs):
        ...
        for row in range(self.rows):
            for col in range(self.columns - 1):
                ...
                outputs["diff"][j] = (
                    inputs["thickness"][i] - inputs["thickness"][i + 1] - delta
                )
                outputs["diff"][j + 1] = (
                    inputs["thickness"][i + 1] - inputs["thickness"][i] - delta
                )
                j += 2
        ...
        for row in range(self.rows - 1):
            for col in range(self.columns):
                ...
                outputs["diff"][j] = (
                    inputs["thickness"][i] - inputs["thickness"][i2] - delta
                )
                outputs["diff"][j + 1] = (
                    inputs["thickness"][i2] - inputs["thickness"][i] - delta
                )
                j += 2
                ...
                if "thickness" in d_inputs:
                    ...
                    for row in range(self.rows):
                        for col in range(self.columns - 1):
                            i = self.columns * row + col
                            d_outputs["diff"][j] += (
                                d_inputs["thickness"][i] - d_inputs["thickness"][i + 1]
                            )
                            d_outputs["diff"][j + 1] += (
                                d_inputs["thickness"][i + 1] - d_inputs["thickness"][i]
                            )
                            j += 2
                    ...
                    for row in range(self.rows - 1):
                        for col in range(self.columns):
                            ...
                            i2 = self.columns * (row + 1) + col
                            d_outputs["diff"][j] += (
                                d_inputs["thickness"][i] - d_inputs["thickness"][i2]
                            )
                            d_outputs["diff"][j + 1] += (
                                d_inputs["thickness"][i2] - d_inputs["thickness"][i]
                            )
                            j += 2
        if mode == "rev":
            if "diff" in d_outputs:
...
if __name__ == "__main__":
    ...
    prob.model.add_subsystem(
        "smoothness_grid", SmoothnessEvaluatorGrid(rows=3, columns=4)
    )
