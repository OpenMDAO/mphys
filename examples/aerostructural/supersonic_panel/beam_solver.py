...
class Beam:
    ...
    def set_adjoint(self, adjoint):
        ...
        if self.rank == 0:
            ...
            Adjoint = self._extract_2dof(
                size=len(adjoint) * 2 // self.n_dof, mask=None, x=adjoint
            )
            Adjoint[np.setdiff1d(np.arange(len(Adjoint)), self.fdof)] = 0.0
    ...
    def compute_stiffness_derivatives(self, u, adjoint):
        ...
        if self.rank == 0:
            ...
            for i in range(len(x) - 1):
                ...
                EI_wrt_dv_struct = (
                    (1 / 12) * self.modulus * self.panel_width * 3 * self.dv_struct**2
                )
                ...
                d_dv_struct[i] += (
                    Adjoint[i * 2 : i * 2 + 4] @ K_wrt_dv_struct @ U[i * 2 : i * 2 + 4]
                )
                ...
                d_x[i + 1] += (
                    Adjoint[i * 2 : i * 2 + 4] @ K_wrt_L @ U[i * 2 : i * 2 + 4]
                )
    ...
    def compute_stress_derivatives(self, u, stress, aggregation_parameter, adjoint):
        ...
        func_struct_wrt_stress = (
            ...
            / self.yield_stress
            / np.sum(np.exp(f * aggregation_parameter))
        )
        ...
        if self.rank == 0:
            ...
            for i in range(len(x) - 1):
                ...
                d_rotation[i + 1] += (
                    adjoint * func_struct_wrt_stress[i] * stress_wrt_rotation
                )
                d_rotation[i] -= (
                    adjoint * func_struct_wrt_stress[i] * stress_wrt_rotation
                )
            ...
            d_yield_stress = adjoint * np.dot(
                np.exp(f * aggregation_parameter)
                / np.sum(np.exp(f * aggregation_parameter)),
                -f / self.yield_stress,
            )
    ...
    def compute_mass_derivatives(self, adjoint):
        ...
        if self.rank == 0:
            ...
            for i in range(len(x) - 1):
                # dv_struct derivatives
                d_dv_struct[i] += (
                    adjoint * (x[i + 1] - x[i]) * self.panel_width * self.density
                )

                # x derivatives
                d_x[i + 1] += (
                    adjoint * self.panel_width * self.dv_struct[i] * self.density
                )
                d_x[i] -= adjoint * self.panel_width * self.dv_struct[i] * self.density

                # density derivatives
                d_density += (
                    adjoint * (x[i + 1] - x[i]) * self.panel_width * self.dv_struct[i]
                )
