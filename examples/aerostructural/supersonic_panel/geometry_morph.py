...
class GeometryMorph(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        for name, n_nodes in zip(self.options['names'], self.options['n_nodes']):
            self.add_input(f'x_{name}0_in', distributed=True, shape_by_conn=True)
            self.add_output(f'x_{name}0', shape=n_nodes*3, distributed=True, tags=['mphys_coordinates'])

    def compute(self,inputs,outputs):
        for name in self.options['names']:
            outputs[f'x_{name}0'] = inputs['geometry_morph_param']*inputs[f'x_{name}0_in']
            ...
            for name in self.options['names']:
                if f'x_{name}0' in d_outputs:
                    if 'geometry_morph_param' in d_inputs:
                        d_inputs['geometry_morph_param'] += self.comm.allreduce(np.sum(d_outputs[f'x_{name}0']*inputs[f'x_{name}0_in']), op=MPI.SUM)
                    
                    if f'x_{name}0_in' in d_inputs:
                        d_inputs[f'x_{name}0_in'] += d_outputs[f'x_{name}0']*inputs['geometry_morph_param'] 
