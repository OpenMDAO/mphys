        ...
        if mode == "rev":
            if "dv_struct" in d_outputs:
                if "lumped_dv" in d_inputs:
                    d_inputs["lumped_dv"] += np.inner(
                        d_outputs["dv_struct"], self.options["dv"]
                    )
            ...
            for i in range(len(self.coordinate_indices)):
                ...
                if self.output_name in d_outputs:
                    if "sweep_dv" in d_inputs:
                        if len(x_0) > 0:
                            d_inputs["sweep_dv"][i] += np.inner(
                                np.c_[Y / self.span, Y * 0, Z * 0].flatten(),
                                d_outputs[self.output_name][self.coordinate_indices[i]],
                            )
                        d_inputs["sweep_dv"][i] = self.comm.allreduce(
                            d_inputs["sweep_dv"][i], op=MPI.SUM
                        )

                    if "span_dv" in d_inputs:
                        if len(x_0) > 0:
                            d_inputs["span_dv"][i] += np.inner(
                                np.c_[X * 0, Y / self.span, Z * 0].flatten(),
                                d_outputs[self.output_name][self.coordinate_indices[i]],
                            )
                        d_inputs["span_dv"][i] = self.comm.allreduce(
                            d_inputs["span_dv"][i], op=MPI.SUM
                        )
        ...
        if len(self.nnodes) == 0:
            ...
            for name, builder in self.builders.items():
                ...
                for body in self.body_tags:
                    ...
                    x_indices = np.sort(
                        np.hstack([grid_ids * 3, grid_ids * 3 + 1, grid_ids * 3 + 2])
                    ).tolist()
                    coordinate_indices[name] += [x_indices]

        geometry_morphing = om.Group()
        for i in range(len(self.nnodes)):
            geometry_morphing.add_subsystem(
                f"{self.names[i]}_geometry_morph",
                WingGeometry(
                    name=self.names[i],
