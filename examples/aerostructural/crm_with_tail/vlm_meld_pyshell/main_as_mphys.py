...
struct_nprocs = min(
    [comm.Get_size(), 15]
)  # number of procs to run the structural solver on
...
if body_option == 1:
    ...
    body_tags_meld = [
        {"aero": [0], "struct": np.arange(240)}
    ]  # couple wing VLM boundary (0) with wing property groups (0-239)
    body_tags_geom = [
        {"aero": [0], "struct": np.arange(240)},
        {"aero": [1], "struct": []},
    ]  # move aero-only tail with geometry builder
    dv_struct = 0.01 * np.ones(240)
    meld_n = 200
    meld_beta = 0.5
elif body_option == 3:
    ...
    body_tags_geom = [
        {"aero": [0], "struct": np.arange(240)},
        {"aero": [], "struct": np.arange(240, 285)},
    ]  # move struct-only tail with geometry builder
...
span = 29.39
sweep_dv = [0.0] * len(body_tags_geom)
span_dv = [0.0] * len(body_tags_geom)
...
class Top(Multipoint):
    def setup(self):
        ...
        if (
            body_option > 2
        ):  # two separate KS functions for wing and tail, based on element number
        ...
        self.add_subsystem(
            "thickness_lumping", LumpDvs(N_dv=len(dv_struct), dv=dv_struct)
        )
        self.thickness_dv.add_output("thickness_dv", 1.0)
        ...
        geom_builder = GeometryBuilder(
            {"aero": aero_builder, "struct": struct_builder},
            span=span,
            body_tags=body_tags_geom,
        )
        self.add_subsystem(
            "geometry", geom_builder.get_mesh_coordinate_subsystem(), promotes=["*"]
        )
        ...
        self.add_subsystem(
            "inertial_loads",
            PyshellInertialForce(gravity=9.81, struct_solver=struct_builder.solver),
        )
