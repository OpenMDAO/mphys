...
from modal_comps import HarmonicForcer, ModalDisplacements, ModalForces, ModalStep
...
class ModalIntegrator(om.ExplicitComponent):
    ...
    def _setup_step_problem(self):
        ...
        model.add_subsystem(
            "modal_forces", ModalForces(nmodes=nmodes, root_name=root_name)
        )
        model.add_subsystem("modal_solver", ModalStep(nmodes=nmodes, dt=dt))
        model.add_subsystem(
            "modal_disps", ModalDisplacements(nmodes=nmodes, root_name=root_name)
        )
    ...
    def _forward_linearized_loop(self, inputs, d_inputs, d_outputs):
        ...
        for var in d_inputs.keys():
            if var == "amp" or var == "freq":
                ...
                dznm_dx[var] = np.zeros(
                    (5, self.options["nmodes"], self.options["nmodes"])
                )

        for step in range(1, self.options["nsteps"] + 1):

            pfpz = (
                np.array([1.0, 0.0]) if step == self.options["nsteps"] else np.zeros(2)
            )
            ...
            for var in d_inputs.keys():
                ...
                for var_index in range(var_length):
                    ...
                    # semi-totals
                    jac_vecs = self.problem.compute_jacvec_product(
                        of=["modal_solver.zn"], wrt=wrt, mode="fwd", seed=seed
                    )

                    dfdx[var][var_index] += pfpz.dot(jac_vecs["modal_solver.zn"])

                    dznm_dx[var][0, :, var_index] = jac_vecs["modal_solver.zn"].reshape(
                        dznm_dx[var][0, :, var_index].shape
                    )
...
def check_integrator_partials():
    ...
    prob.model.add_subsystem(
        "modal_integrator",
        ModalIntegrator(nmodes=2, nsteps=10, dt=1.0, root_name="sphere_body1"),
    )
    prob.setup()
