...
for step in range(1, nsteps + 1):
    ...
    # coupling loop
    model.connect(
        f"{step_group_name}.modal_forces.mf", f"{step_group_name}.modal_solver.f"
    )
    model.connect(
        f"{step_group_name}.modal_solver.zn", f"{step_group_name}.modal_disps.md"
    )
    ...
    for backplane in range(1, 5):
        ...
        if back_step > 0:
            model.connect(
                f"Step{back_step}.modal_solver.zn",
                f"Step{step}.modal_solver.znm{backplane}",
            )
        else:
...
prob.check_totals(
    of=[f"Step{nsteps}.modal_solver.zn"],
    wrt=["indeps.k", "indeps.m", "indeps.z0", "indeps.amp"],
)
