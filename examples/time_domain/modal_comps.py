...
class ModalStep(om.ImplicitComponent):
    ...
    def solve_linear(self, d_outputs, d_residuals, mode):
        ...
        if mode == "fwd":
            d_outputs["zn"] = d_residuals["zn"] / (
                self.beta[0] * m + self.alpha[0] * c + k
            )
        if mode == "rev":
            d_residuals["zn"] = d_outputs["zn"] / (
                self.beta[0] * m + self.alpha[0] * c + k
            )
    ...
    def apply_linear(self, inputs, outputs, d_inputs, d_outputs, d_residuals, mode):
        ...
        if mode == "fwd":
            if "zn" in d_residuals:
                ...
                if "znm2" in d_inputs:
                    d_residuals["zn"] += (
                        self.beta[2] * m * d_inputs["znm2"]
                        + self.alpha[2] * c * d_inputs["znm2"]
                    )
                if "znm1" in d_inputs:
                    d_residuals["zn"] += (
                        self.beta[1] * m * d_inputs["znm1"]
        ...
        if mode == "rev":
            if "zn" in d_residuals:
                ...
                if "znm2" in d_inputs:
                    d_inputs["znm2"] += (
                        self.beta[2] * m * d_residuals["zn"]
                        + self.alpha[2] * c * d_residuals["zn"]
                    )
                if "znm1" in d_inputs:
                    d_inputs["znm1"] += (
                        self.beta[1] * m * d_residuals["zn"]
                        + self.alpha[1] * c * d_residuals["zn"]
                    )


class ModalInterface(om.ExplicitComponent):
    def initialize(self):
        self.options.declare("nmodes", default=1)
    ...
    def compute(self, inputs, outputs):
        ...
        for imode in range(self.options["nmodes"]):
            for inode in range(self.nnodes):
                for k in range(3):
                    outputs["mf"][imode] += (
                        self.mdisp[imode, inode, k] * inputs["f"][3 * inode + k]
                    )
                ...
                if "f" in d_inputs:
                    for imode in range(self.options["nmodes"]):
                        for inode in range(self.nnodes):
                            for k in range(3):
                                d_inputs["f"][3 * inode + k] += (
                                    self.mdisp[imode, inode, k] * d_outputs["mf"][imode]
                                )
...
class ModalDisplacements(ModalInterface):
    ...
    def compute(self, inputs, outputs):
        ...
        for imode in range(self.options["nmodes"]):
            for inode in range(self.nnodes):
                for k in range(3):
                    outputs["dx"][3 * inode + k] += (
                        self.mdisp[imode, inode, k] * inputs["md"][imode]
                    )
            ...
            if "dx" in d_outputs:
                if "md" in d_inputs:
                    for imode in range(self.options["nmodes"]):
                        for inode in range(self.nnodes):
                            for k in range(3):
                                d_outputs["dx"][3 * inode + k] += (
                                    self.mdisp[imode, inode, k] * d_inputs["md"][imode]
                                )
        if mode == "rev":
            if "dx" in d_outputs:
                if "md" in d_inputs:
    ...
    def compute_jacvec_product(self, inputs, d_inputs, d_outputs, mode):
        ...
        if mode == "fwd":
            if "f" in d_outputs:
                if "amp" in d_inputs:
                    d_outputs["f"] += (
                        np.sin(freq * time) * np.ones(self.nnodes * 3) * d_inputs["amp"]
                    )
                if "freq" in d_inputs:
                    d_outputs["f"] += (
                        time
        ...
        if mode == "rev":
            if "f" in d_outputs:
                ...
                if "freq" in d_inputs:
                    d_inputs["freq"] += (
                        time * np.cos(freq * time) * np.sum(d_outputs["f"])
                    )
                if "time" in d_inputs:
                    d_inputs["time"] += (
                        freq * np.cos(freq * time) * np.sum(d_outputs["f"])
                    )
                if "dx" in d_inputs:
                    d_inputs["dx"] -= self.c1 * d_outputs["f"]


def check_modal_partials():
    prob = om.Problem()
    prob.model.add_subsystem(
        "modal_force", ModalForces(nmodes=2, root_name="sphere_body1")
    )
    prob.model.add_subsystem("modal_step", ModalStep(nmodes=2, dt=0.1))
    prob.model.add_subsystem(
        "modal_disp", ModalDisplacements(nmodes=2, root_name="sphere_body1")
    )
    prob.model.add_subsystem(
        "harmonic_forcer", HarmonicForcer(root_name="sphere_body1")
    )
